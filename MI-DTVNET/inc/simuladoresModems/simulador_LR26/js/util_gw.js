...
// for WPS ---------------------------------------------------->>
var wps_warn1='SSID configurada por WPS. Cualquier cambio en la configuración ' +
				'podría causar la desconexión de las estaciones. ' + 
				'¿Está seguro de que desea continuar con la nueva configuración?';
var wps_warn2='Punto de acceso Mode had been configured by WPS. Any change of the setting ' +
				'podría causar la desconexión de las estaciones. ' + 
				'¿Está seguro de que desea continuar con la nueva configuración?';
var wps_warn3='Configuración de seguridad ajustada por WPS. Cualquier cambio en la configuración ' +
				'podría causar la desconexión de las estaciones. ' + 
				'¿Está seguro de que desea continuar con la nueva configuración?';
var wps_warn4='WPS no admite la autenticación empresarial WPA. ' +
				'La utilización de esta configuración provocará la desactivación de WPS. ' + 
				'¿Está seguro de que desea continuar con la nueva configuración?';
var wps_warn5='WPS no admite la autenticación 802.1x. ' +
				'La utilización de esta configuración provocará la desactivación de WPS. ' + 
				'¿Está seguro de que desea continuar con la nueva configuración?';
var wps_warn6='WPS no admite el modo WDS. ' +
				'La utilización de esta configuración provocará la desactivación de WPS. ' + 
				'¿Está seguro de que desea continuar con la nueva configuración?';
var wps_warn7='WPS no admite el modo de cliente ahdoc. ' +
				'La utilización de esta configuración provocará la desactivación de WPS. ' + 
				'¿Está seguro de que desea continuar con la nueva configuración?';
var encrypt_11n = '¡Modo de cifrado no válido! WPA o WPA2, AES de conjunto de cifrado deberá utilizarse para la banda 802.11n.';
var encrypt_basic = 'El modo de cifrado no es adecuado para la banda 802.11n. Modifique la configuración de cifrado de WLAN o no funcionará de forma correcta.';
var encrypt_confirm='¿Está seguro de que desea continuar con este modo de cifrado para la banda 11n? Es posible que no se obtenga un rendimiento correcto cuando el usuario está utilizando la red WLAN.';
...
//----------------------Fourth pppoe info check  End----------------------------------		  
	...
	if((!form.pptpServerIpAddr.disabled)&&
	  ...
	  if (form.pptpPassword.value=="") {
		  alert('Contraseña cannot be empty!');
...
// --------------- USB3G wanType ---------------
        ...
        if (form.USB3G_APN.value=="") {
            alert('Punto de accesoN name cannot be empty!');
...
function enableWLAN(form, wlan_id)
{
  ...
  else
  {
	...
	if(band_value == 9 || band_value ==10 || band_value==7 || band_value==11 || band_value==14 || band_value==63 || band_value==71 || band_value==75){ //8812
...
/* RUSSIAN */		reg_chan_8812_full[11]= new Array("36","40","44","48");                                                                                                  
...
var currentBand;		
	...
	if(band_value==9 || band_value==10 || band_value ==7){	 // 8812 ?? adjust channel ??
		if(bound ==0)
			adjust_chan=0;
		if(bound ==1)
			adjust_chan=1;	
...
function check_wpa_psk(form, wlan_id)
{
	...
	if (form.elements["pskFormat"+wlan_id].selectedIndex==1) {
		if (str.length != 64) {
			alert('Clave precompartida value should be 64 characters.');
		...
		if (str.length < 8) {
			alert('Clave precompartida value should be set at least 8 characters.');
			form.elements["pskValue"+wlan_id].focus();
			return false;
		}
		if (str.length > 63) {
			alert('Clave precompartida value should be less than 64 characters.');
			form.elements["pskValue"+wlan_id].focus();
			return false;
		}
	}
...
function checkIPMask(field)
{
  ...
  if (field.value=="") {
      	alert("¡La máscara de subred no puede estar vacía! Se la deberá completar con números de 4 dígitos como xxx.xxx.xxx.xxx.");
	field.value = field.defaultValue;
	field.focus();
	return false;
  }
  if ( validateKey( field.value ) == 0 ) {
      	alert("Valor de máscara de subred no válido. Deberá ser un número decimal (0-9).");
      	field.value = field.defaultValue;
	field.focus();
	return false;
  }
...
ntp_zone_array[10]=new ntp_entry("(GMT-05:00)Bogotá, Lima, Quito", "5 1");
ntp_zone_array[11]=new ntp_entry("(GMT-05:00)Hora del Este (EE. UU. y Canadá)", "5 2");
ntp_zone_array[12]=new ntp_entry("(GMT-05:00)Indiana (Este)", "5 3");
ntp_zone_array[13]=new ntp_entry("(GMT-04:00)Hora del Atlántico (Canadá)", "4 1");
ntp_zone_array[14]=new ntp_entry("(GMT-04:00)Caracas, La Paz", "4 2");
ntp_zone_array[15]=new ntp_entry("(GMT-04:00)Santiago", "4 3");
ntp_zone_array[16]=new ntp_entry("(GMT-03:30)Terranova", "3 1");
...
ntp_zone_array[19]=new ntp_entry("(GMT-02:00)Atlántico Medio", "2 1");
...
function showcontrolsideband_updated(form, band, wlan_id, rf_num, index)
{
  ...
  {
	 form.elements["controlsideband"+wlan_id].options[idx++] = new Option("Superior", "0", false, false);
	 form.elements["controlsideband"+wlan_id].options[idx++] = new Option("Inferior", "1", false, false);
  }
...
function showchannelbound_updated(form, band, wlan_id, rf_num)
{
 ...
 form.elements["channelbound"+wlan_id].options[idx++] = new Option("20 MHz", "0", false, false);
 form.elements["channelbound"+wlan_id].options[idx++] = new Option("40 MHz", "1", false, false);
 
 if(band == 75 || band ==71|| band ==63){
